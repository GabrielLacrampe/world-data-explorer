...
export default function EconomyTab({ data, worldBankData, worldBankLoading, staticData, countryCode, subtab }) {
  ...
  const wbValue = (key, format, unit) =>
    wb === null && worldBankLoading ? 'Loading…' : formatIndicatorValue(wb?.[key], format, unit)
  ...
  if (subtab === 'overview') return (
    <div className="flex flex-col gap-5">
      ...
      <Section title="People">
        <DataRow label="Population"      value={wb?.['SP.POP.TOTL'] ? Math.round(wb['SP.POP.TOTL']).toLocaleString('en-US') : (wb === null && worldBankLoading ? 'Loading…' : 'N/A')} />
        <DataRow label="Life Expectancy" value={wbValue('SP.DYN.LE00.IN', 'decimal', 'years')} />
        <DataRow label="Literacy Rate"   value={wbValue('SE.ADT.LITR.ZS', 'percent', '%')} />
        <DataRow label="Internet Users"  value={wbValue('IT.NET.USER.ZS', 'percent', '%')} />
      </Section>
      <Section title="Economy">
        <DataRow label="Currency"
          value={Object.values(data.currencies ?? {}).map((c) => `${c.name} (${c.symbol})`).join(', ') || 'N/A'} />
        <DataRow label="GDP per Capita" value={wbValue('NY.GDP.PCAP.CD', 'currency', 'USD')} />
        <DataRow label="GDP Growth"     value={wbValue('NY.GDP.MKTP.KD.ZG', 'percent', '%')} />
        <DataRow label="Unemployment"   value={wbValue('SL.UEM.TOTL.ZS', 'percent', '%')} />
        <DataRow label="Inflation"      value={wbValue('FP.CPI.TOTC.ZG', 'percent', '%')} />
        <DataRow label="Gini Index"     value={wbValue('SI.POV.GINI', 'decimal', '')} />
      </Section>
  ...
  if (subtab === 'environ') return (
    <div className="flex flex-col gap-5">
      <Section title="Environment">
        <DataRow label="CO₂ per Capita"     value={wbValue('EN.ATM.CO2E.PC', 'decimal', 'tonnes')} />
        <DataRow label="Electricity Access" value={wbValue('EG.ELC.ACCS.ZS', 'percent', '%')} />
        <DataRow label="Renewable Energy"   value={wbValue('EG.FEC.RNEW.ZS', 'percent', '%')} />

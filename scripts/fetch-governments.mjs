...
// ── Also generate gov-photos.sql for Supabase ────────────────────────────────
// Build ISO3→ISO2 map from country_codes.sql
const iso3toIso2 = {}
const ccSql = readFileSync('csv-data/country_codes.sql', 'utf8')
for (const m of ccSql.matchAll(/'([A-Z]{3})',\s*'([A-Z]{2})'/g)) {
  iso3toIso2[m[1]] = m[2]
}
const iso2toIso3 = Object.fromEntries(Object.entries(iso3toIso2).map(([k, v]) => [v, k]))

const sqlLines = [
  '-- Gov leader photos — generated by scripts/fetch-governments.mjs (Wikidata)',
  '-- Run this in Supabase SQL editor after importing gov.csv.',
  '',
]
for (const [iso2, entry] of Object.entries(out)) {
  const iso3 = iso2toIso3[iso2]
  if (!iso3) continue
  const hosPhoto = entry.headOfState?.photo
  const hogPhoto = entry.headOfGov?.photo
  if (!hosPhoto && !hogPhoto) continue
  const parts = []
  if (hosPhoto) parts.push(`head_of_state_photo = '${hosPhoto.replace(/'/g, "''")}'`)
  if (hogPhoto) parts.push(`head_of_government_photo = '${hogPhoto.replace(/'/g, "''")}'`)
  sqlLines.push(`UPDATE governments SET ${parts.join(', ')} WHERE iso3 = '${iso3}';`)
}

const sqlPath = 'csv-data/gov-photos.sql'
writeFileSync(sqlPath, sqlLines.join('\n') + '\n')
console.log(`Written ${sqlLines.length - 3} photo UPDATEs to ${sqlPath}`)
